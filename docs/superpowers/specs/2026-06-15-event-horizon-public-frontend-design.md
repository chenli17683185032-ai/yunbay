...
Implementation depth: homepage-only raw WebGL point-cloud morphing hero plus lightweight CSS public shell
...
6. Do not introduce heavyweight site-wide WebGL or Three.js. Raw WebGL is allowed only for the homepage point-cloud hero.
...
## 4.1 Revised homepage art direction

The homepage hero is no longer a static Event Horizon background. It must be an Active Theory-like point-cloud morphing artwork. The artwork is the primary visual experience on `/`, not a side decoration.

Runtime morph states:

- selected face point cloud, closed-eye when the current user is not logged in or has zero balance, open-eye when the current user has positive quota,
- generated black-hole point cloud, 30,000 points,
- generated high-end external-B-site-style abstract point cloud, 30,000 points.

The engine must support free seamless morphing among the three runtime states. The face state is chosen from user balance, then the hero cycles among: selected face, black hole, abstract point-cloud sculpture.

Balance rule:

- no user: closed-eye face, black hole, abstract point cloud,
- user quota less than or equal to zero: closed-eye face, black hole, abstract point cloud,
- user quota greater than zero: open-eye face, black hole, abstract point cloud.

The black-hole and abstract point-cloud assets are generated by the project. The generated assets should be deterministic, committed, and not created randomly at runtime. Face assets may be derived from the supplied OBJ source and normalized to the same 30,000-point count for morph compatibility.
...
### 6.8 Point-cloud morphing hero architecture

Create a homepage-only point-cloud module under `web/default/src/features/home/point-cloud/`. It should keep generation, runtime state selection, and rendering responsibilities separate.

Required units:

- asset manifest: describes generated point-cloud assets and point count,
- balance state helper: maps `auth.user?.quota` to `closed` or `open`,
- morph sequence helper: returns the three runtime states for the current balance,
- WebGL renderer component: loads assets, uploads target positions to GPU buffers, morphs between shapes, and handles pointer disturbance,
- CSS fallback/poster: shown while loading, on WebGL failure, or under constrained motion.

The point-cloud component must not import authenticated console modules. It may read `auth.user?.quota` from the existing auth store only at the homepage boundary and pass a simple face-state prop into the renderer.

All morph targets must have the same point count. Use 30,000 points for all committed generated assets so morphing can be done by interpolating matching array indices.
...
- Use raw WebGL only inside the homepage point-cloud morph component. Do not use GSAP in this pass.


### 7.1 Point-cloud interaction plan

The point-cloud renderer should provide:

- automatic slow cycling among three runtime states,
- click or pointer-down morph advance,
- pointer disturbance that repels or bends nearby particles,
- different visual feel between face and black-hole states through color, scale, and morph timing,
- smooth interpolation with eased progress rather than hard swaps.

Reduced-motion behavior:

- no automatic cycling,
- no pointer disturbance beyond a subtle hover highlight,
- render the selected face state or a static fallback poster.
...
8. Homepage point-cloud verification:
   - generated manifest contains black-hole and abstract assets at 30,000 points,
   - face, black-hole, and abstract assets share the same point count,
   - quota state helper maps zero balance to closed face and positive balance to open face,
   - morph sequence helper returns three states for both balance cases,
   - homepage renders without blocking public navigation when WebGL fails.
...
- Use committed point-cloud binaries and one homepage-only raw WebGL canvas. Avoid WebGL on other public or authenticated pages.
