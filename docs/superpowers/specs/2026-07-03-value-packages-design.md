...
这个配置必须细化到“每一张套餐卡片”这一层：日卡、周卡、月卡各自拥有独立的联动小铺付款链接和商品信息。不能只配置一个全局联动小铺链接，也不能要求管理员通过外部表格或代码改配置。

用户购买时必须直接调用站内现有支付系统来创建付款会话：

1. 前端只提交用户选择的套餐计划 ID；
2. 后端根据该套餐卡片保存的联动小铺付款链接、商品名称、价格等配置创建 LDXP / 联动小铺支付会话；
3. 继续复用现有 LDXP session、worker 拉单、二维码展示、邮件核验和成功结算链路；
4. 前端展示现有支付弹窗 / 支付等待流程，而不是把用户简单跳转到一个静态外链后让用户手工联系管理员；
5. 支付成功后进入套餐订单结算分支，开通 / 续费 / 覆盖套餐，不增加钱包余额。
...
管理界面必须以“日卡 / 周卡 / 月卡三张卡片”为核心配置对象。管理员进入页面后应能直接看到三张套餐卡当前是否启用、价格、有效期、模型分组、并发、限额以及联动小铺付款配置是否完整，而不是只看到一张通用订阅表格后再猜哪条记录对应哪张卡。
...
管理员保存后，用户端购买按钮必须直接使用该卡片的配置调用站内现有 LDXP / 联动小铺支付系统。管理界面不应只保存一个展示链接；它保存的配置必须足够让后端创建真实付款会话。
...
- 价格不能为负；
- 有效期必须大于 0；
- 若套餐启用，则联动小铺付款链接、商品名称和付款金额必须完整，否则只能保存为禁用或草稿状态。

### 12.3 管理界面卡片配置体验

管理员配置页建议采用“卡片总览 + 右侧抽屉编辑”的体验：

1. 总览区固定展示日卡、周卡、月卡三张配置卡；
2. 每张卡展示启用状态、价格、有效期、模型分组、并发、5 小时限额、7 天限额、联动小铺配置状态；
3. 点击“编辑”打开配置抽屉，抽屉内包含基础信息、权益、模型分组、限额、联动小铺付款配置；
4. 联动小铺配置区必须能直接粘贴或编辑该卡片专属付款链接；
5. 配置不完整时卡片显示醒目的“付款未配置”，用户端购买按钮不可用；
6. 配置完整且启用后，用户端购买按钮调用现有支付系统创建付款会话；
7. 管理员应能一眼看出哪张卡缺少付款链接、哪张卡缺少并发或限额配置。

管理界面不允许出现以下降级设计：

- 只提供一个全局联动小铺链接，三张卡共用；
- 只让管理员填写文字说明，不参与支付会话创建；
- 让用户点击购买后跳外链付款，但站内无法跟踪订单和自动开通；
- 并发、5 小时限额、7 天限额需要改代码或改数据库才能配置。
...
| `ldxp_product_amount` | `decimal(10,6)` | 该卡片实际调用联动小铺支付系统时使用的付款金额 |
| `ldxp_product_ref` | `varchar(128)` | 可选外部商品 ID |
| `ldxp_session_ttl_seconds` | `bigint` | 可选：该卡片支付会话有效期；为空时复用现有 LDXP 默认 TTL |
...
- LDXP 商品字段默认空；若套餐启用，后台必须要求这些字段补齐后才能对用户开放购买。
...
- 支付会话有效期；
- 配置完整性提示；
- “保存后用户购买将直接调用现有联动小铺支付系统”的说明。

用户端不应读取这个链接后直接 `window.location.href` 跳转；用户购买动作应调用后端创建支付会话，由后端复用现有 LDXP 支付系统和订单核验流程。
...
- 管理员发布任意一张套餐卡前，必须能在管理界面完成这张卡自己的联动小铺付款链接配置，并且用户购买时直接调用站内现有支付系统。
...
- 管理界面必须细化到每张套餐卡可配置自己的联动小铺付款链接，用户购买时直接调用现有 LDXP / 联动小铺支付系统创建付款会话。
