...
- 最终会话为 `canceled`，`worker_detected_time/verified_time/redeemed_time/topup_id/redemption_id` 全为 0；不存在 `ldxp:LD260724SG04DI` 的 `top_ups` 或关联兑换记录，所以不是前端余额缓存，100 刀确实未入账。
- 生产 `LDXP_RELEASE_SLOT_AFTER_QR=false`、`LDXP_REQUIRE_MAIL_MATCH=false` 均符合既有合同。当前 worker IMAP 未完整配置、mail poller 禁用，因此取消活页面后没有第二条邮件恢复信号。
- 同一用户随后订单 `LD2607241QLL75` 在 11:11:41 正常 `result posted` 并创建成功 `top_ups.id=70`，证明当时 worker、回国代理和后端入账事务可用；该 10 刀成功记录不是 100 刀订单的补偿。
- 根因是取消语义与外部支付生命周期不一致：`qr_ready` 页面仍提供无二次确认的“取消”，它会终止云贝会话，却不能撤销已创建的 LDXP/支付宝订单。用户扫码后取消会形成“外部可继续支付、内部停止观测”的孤儿订单。
- 本轮只读，没有补款、重试、数据库/配置修改、容器重启或部署。处理本单前应再次核对渠道未退款与 `top_ups.trade_no` 不存在，再通过幂等管理路径补 100 刀。长期修复应拆分“关闭窗口”和“放弃订单”，`qr_ready` 后关闭 UI 不取消后台观测，并补充有界对账/恢复通道。
...
测活补充：当前分组内所有 active、schedulable 中转站和自有账号都会调用 Sub2API 自带的 `/admin/accounts/:id/test`。报告逐账号显示成功/失败和完整请求延迟。中转站不计算用量，只检查存活和延迟；只有连续 3 次检查（约 15 分钟）中所有可用中转站均超过 15 秒、且不存在其他不高于 15 秒的可用渠道时才告警。

一键状态报告的开头现增加汇总区：自有账号显示可用数与单账号剩余范围；只有存在可信容量权重时才显示加权总体剩余，否则明确说明上游未返回绝对总额度，禁止用等权平均伪造总量。中转站显示可用数量与本轮所有测活成功中转站的平均延迟，随后再列逐账号明细。

按用户要求，生产当前采用容量估算进行加权汇总：账号 9、452、66 各估算 `15M`，账号 471 估算 `200M`。报告必须明确标记“估算总体剩余/估算总量”，不能把该结果表述为上游官方绝对额度。当前百分比为 1%、3%、1%、54% 时，估算总量 `245M`、已用约 `108.8M`、剩余约 `136.2M`，总体剩余约 `55.6%`。

## 2026-07-12 安全使用分组误报修正

new-api 使用的 Key 已从 `自建池` 切换到 `自建安全使用`。该组没有中转站账号，报告和邮件在此模式下不再出现“中转站”汇总或测试字段，只展示自有账号总体美元估算、逐账号剩余、测活和延迟。

本次异常邮件并非额度达到 80%，而是安全使用组新增自有账号 `abodes.peso.4q@icloud.com`（ID 472）后，旧私有容量映射仍指向已移出的 ID 471，脚本把“缺少容量配置”误作为每 5 分钟紧急异常。已暂停旧 cron、将 2K 美金额度估算迁移到 ID 472、清除错误告警状态并恢复 cron。修正后实时结果：估算总额 `$245.00`、已用约 `$20.80`、剩余约 `$224.20`（91.5%），4/4 自有账号测活成功，因此不告警。
