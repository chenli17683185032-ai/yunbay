...
func migrateDB() error {
	...
	err := DB.AutoMigrate(
		...
		&ValuePackageQuotaMigrationReceipt{},
		&SVIPValidTopupReconcileReceipt{},
...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&TopUp{}, "TopUp"},
		{&OrderDeletionMark{}, "OrderDeletionMark"},
		...
		{&ValuePackageQuotaMigrationReceipt{}, "ValuePackageQuotaMigrationReceipt"},
		{&SVIPValidTopupReconcileReceipt{}, "SVIPValidTopupReconcileReceipt"},
...
` + "`gift_reset_count`" + ` int NOT NULL DEFAULT 0,
` + "`created_at`" + ` bigint,
` + "`updated_at`" + ` bigint,
PRIMARY KEY (` + "`id`" + `)
)`
...
func ensureSubscriptionOrderTableSQLite() error {
	...
	required := []sqliteColumnDef{
		...
		{Name: "gift_reset_count", DDL: "`gift_reset_count` integer NOT NULL DEFAULT 0"},
	}
	for _, col := range required {
		if _, ok := existing[col.Name]; ok {
			continue
