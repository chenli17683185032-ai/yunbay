...
func ldxpMailEventFromIMAPMessage(sourceAccount string, message *imapclient.FetchMessageBuffer, raw []byte) (*model.LdxpMailEvent, error) {
	...
	event := &model.LdxpMailEvent{
		...
		ContentMasked: parsed.ContentMasked,
		ParseStatus:   "parsed",
		CreatedTime:   now,
	}
	if message != nil {
		...
		if message.Envelope != nil {
			event.MessageId = message.Envelope.MessageID
			event.Subject = message.Envelope.Subject
			if len(message.Envelope.From) > 0 {
				event.FromAddress = message.Envelope.From[0].Addr()
			}
			if !message.Envelope.Date.IsZero() {
				event.PaymentTime = firstNonZero(event.PaymentTime, message.Envelope.Date.Unix())
			}
		}
	}
	return event, nil
