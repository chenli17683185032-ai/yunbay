...
function LocalOrderCell({ row }: { row: OrderManagementOrderItem }) {
  return (
    <div className='flex min-w-48 flex-col gap-1'>
      <span className='font-medium'>{row.session_id || '-'}</span>
      <span className='text-muted-foreground text-xs'>{row.order_type}</span>
    </div>
  )
}
              ...
              {isLoading ? (
                ...
                rows.map((row) => (
                  ...
                  >
                    ...
                    <TableCell>{formatCny(row.external_paid_amount)}</TableCell>
