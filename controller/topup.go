...
func EpayNotify(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 webhook 收到请求 path=%q client_ip=%s method=%s param_keys=%d", c.Request.RequestURI, c.ClientIP(), c.Request.Method, len(params)))
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			if _, upgradeErr := model.MaybeUpgradeUserToVIP(topUp.UserId); upgradeErr != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 VIP 升级检查失败 trade_no=%s user_id=%d error=%q", topUp.TradeNo, topUp.UserId, upgradeErr.Error()))
			}
