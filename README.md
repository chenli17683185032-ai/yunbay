...
| 2026-07-12 | Codex | new-api 生产代码基线 `04f4c32c`；sub2api `0.1.151`；本行所在提交仅为后续台账整理 | 完成超值套餐重置语义、周期 epoch、reconciliation 与退款夹具修复；订单管理页删除重复实时用量模块；固定 sub2api release digest 并增加防回退门禁 | 后端、维护 CLI、并发退款与 race 测试通过；default 前端 typecheck/build/i18n/定向测试通过；主线 CI run `29156729212` 成功；生产 19 条套餐修正为 14 条 `$90` 周卡和 5 条 `$440` 月卡，无已用额度超总额记录 | new-api 已部署 `04f4c32c`；sub2api 已更新到 `0.1.151` 且启用后台模式；使用服务器端独立 watchdog、有界自动回滚和独立服务切换，实际公网中断约 15 秒，未重启无关容器 | 后续部署禁止无限等待 drain、全栈 compose down/up 或无 watchdog 切换；台账文档提交会推进 GitHub `main`，但不代表生产应用重新发布 |
